...
class shimdb_imdb_grab{
    ...
    function title($id){
        ...
        if(isset($title->item(0)->nodeValue)){
            ...
            $data["title"] =esc_sql(trim($tt[0],chr(0xC2).chr(0xA0)));
        }
            ...
            if(isset($year)){
                foreach($year as $y){
                    ...
                    if (strpos($check, 'releaseinfo') !== false) {
                        $data['year'] = isset($y->nodeValue) ? esc_sql(trim($y->nodeValue)) : "";
                    }
                }
            }
        ...
        if(isset($genre)){
            ...
            foreach ($genre as $g){
                ...
                if (strpos($check_genre, 'genres=') !== false) {
                    $genres[$x] = esc_sql(trim($g->nodeValue));
                    $x++;
                }
            }
        ...
        $data['rating'] = isset($rating->item(0)->nodeValue) ? esc_sql(trim($rating->item(0)->nodeValue)) : "?.?";
        ...
        @$data['sum'] = isset($sum->item(0)->nodeValue) ? esc_sql(trim($sum->item(0)->nodeValue)) : "";

        /* Time */
        @$tm = $xpath->query('//time');
        $data['runtime'] = isset($tm->item(0)->nodeValue) ? esc_sql(trim($tm->item(0)->nodeValue)) : "";
        $data['runtime2'] = isset($tm->item(1)->nodeValue) ? esc_sql(trim($tm->item(1)->nodeValue)) : "";
        ...
        if(isset($country)) {
            foreach ($country as $c) {
                ...
                if (strpos($check, 'country_of_origin') !== false) {
                    $countries[] = esc_sql(trim($c->nodeValue));
                }

                if (strpos($check, 'primary_language') !== false) {
                    $langs[] = esc_sql(trim($c->nodeValue));
                }
            }
        ...
        if(isset($sum_crew)){
            ...
            foreach ($sum_crew as $s){
                ...
                if (strpos($check_crew, '/?ref_=tt_ov_dr') !== false) {
                    $directors[] = '<a href="'.esc_url('http://www.imdb.com'.$check_crew).'" target="_blank">'. esc_sql($s->nodeValue).'</a>';
                }

                /* Writers */
                else if(strpos($check_crew, '/?ref_=tt_ov_wr') !== false && strpos($s->nodeValue, 'more credits') == false){
                    $writers[] = '<a href="'.esc_url('http://www.imdb.com'.$check_crew).'" target="_blank">'. esc_sql($s->nodeValue).'</a>';
                }

                /* Stars */
                else if((strpos($check_crew, 'tt_ov_st') !== false) AND (strpos($s->nodeValue, 'full cast') == false)){
                    $stars[] = '<a href="'.esc_url('http://www.imdb.com'.$check_crew).'" target="_blank">'. esc_sql($s->nodeValue).'</a>';
                }
            }
    ...
    function name($id){
        ...
        $data["name"] = isset($name->item(0)->nodeValue) ? esc_sql(trim($name->item(0)->nodeValue,chr(0xC2).chr(0xA0))) : "???";
        ...
        $data['death'] = esc_html($data['death']);
        //BIO
